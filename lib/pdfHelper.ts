...
export const buildInvoicePDF = (invoice: Invoice): jsPDF => {
  ...
  // Title / Invoice No Only in White
  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(20);
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.text(`Invoice No: ${invoice.invoiceNumber}`, 15, 23);

  const creditTermsLabel = invoice.status === "Credit"
    ? `Credit Terms: ${invoice.creditDays ? `${invoice.creditDays} Days` : "Credit"}`
    : `Payment Status: ${invoice.status || "Paid"}`;
  doc.text(creditTermsLabel, 195, 23, { align: "right" });
  ...
  doc.setFont("helvetica", "normal");
  doc.text("Subtotal:", 135, yPos);
  doc.text(`${invoice.subtotal.toFixed(2)}`, 195, yPos, { align: "right" });

  if (invoice.discountTotal > 0) {
    yPos += 6;
    doc.text("Discount Total:", 135, yPos);
    doc.text(`-${invoice.discountTotal.toFixed(2)}`, 195, yPos, { align: "right" });
  }
  ...
  doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
  doc.text("Grand Total (AED):", 135, yPos);
  doc.text(`${invoice.grandTotal.toFixed(2)}`, 195, yPos, { align: "right" });
  ...
  if (invoice.notes && invoice.notes.trim() !== "") {
    ...
    doc.setFont("helvetica", "italic");
    const splitNotes = doc.splitTextToSize(invoice.notes.trim(), 180);
    doc.text(splitNotes, 15, yPos + 5);
  ...
  doc.setFont("helvetica", "normal");
  const creditTermsText = invoice.status === "Credit"
    ? `Credit Terms: ${invoice.creditDays ? `${invoice.creditDays} Days` : "Credit"}`
    : `Payment Status: ${invoice.status || "Paid"}`;
  doc.text(`This is a computer generated invoice. ${creditTermsText}`, 15, yPos);
